...
def auto_sync_check():
    ...
    try:
        if not LOCAL_DB_PATH.exists():
            logging.warning(f"Fichier source introuvable pour auto-sync: {LOCAL_DB_PATH}")
            return False

        if not WEBAPP_DB_PATH.exists():
            logging.info("Fichier webapp inexistant, synchronisation nécessaire")
        ...
        if local_mtime > webapp_mtime:
            logging.info("Fichier local plus récent, synchronisation automatique...")
            return sync_database()
        else:
            logging.info("Bases synchronisées, aucune action nécessaire")
            return True

    except Exception as e:
        logging.error(f"Erreur lors de la vérification auto-sync: {e}")
        return False
